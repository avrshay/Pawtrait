...
4. **Useful commands**
   ```bash
   npx sequelize-cli db:migrate:status   # see which migrations have run
...
Create `backend/.env` (copy the template below — this file is git-ignored, so it's never committed):
...
# Optional — only needed if your local Ollama isn't on the default address/model
...
The frontend has no `.env` — its API/Socket.IO base URLs are hardcoded to `http://localhost:3000` (see Installation above).
...
- **Model**: a **local [Ollama](https://ollama.com/)** server (`http://localhost:11434` by default) — not a paid cloud API, so no API key/billing is involved. [`backend/src/chat/aiAgent.js`](backend/src/chat/aiAgent.js) calls Ollama's `/api/chat` endpoint directly with `fetch`.
